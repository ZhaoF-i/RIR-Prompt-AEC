...
class NET(nn.Module):
    def __init__(self, order=10, channels=20):
        ...
        self.elu = nn.ELU()
        self.rir_linear1 = nn.Linear(50, 20)
        self.rir_linear2 = nn.Linear(20, 1)
    ...
    def forward(self, x, rir): # x:[batch, channel, frequency, time]
    # def forward(self, x):
        # rir = torch.zeros(x.shape[0], 8000).to(x.device)
        comp_rir = self.stft(rir.unsqueeze(dim=1))
        comp_rir = torch.mean(comp_rir, dim=-1, keepdim=True)
        comp_rir = comp_rir.repeat(1, 2, 1, 1)
        X0 = self.stft(x)
        X0 = torch.cat([comp_rir, X0], dim=-1)
