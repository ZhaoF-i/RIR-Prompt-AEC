...
from nnstruct.alpha_predictor import *
...
class NET(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.istft(X0 * mask, t=x.shape[-1])[:, 0]
...
def complexity():
    ...
    mac, param = get_model_complexity_info(model, (16000,), as_strings=True, print_per_layer_stat=True, verbose=True)
    print(mac, param)
    '''
    838.58 MMac 119.9 k
    '''
