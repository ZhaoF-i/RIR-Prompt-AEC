...
class NET(nn.Module):
    def __init__(self, order=10, channels=20):
        ...
        self.cfb_e2 = CFB(channels + 2, channels)
        self.cfb_e3 = CFB(channels + 2, channels)
        self.cfb_e4 = CFB(channels + 2, channels)
        self.cfb_e5 = CFB(channels + 2, channels)
    ...
    # def forward(self, x):
        ...
        comp_rir = self.stft(rir.unsqueeze(dim=1))
        rir_embedding = self.rir_encoder(comp_rir)

        X0 = self.stft(x)
        mix_comp = torch.stack([X0[:, 0], X0[:, 2]], dim=1)
        far_comp = torch.stack([X0[:, 1], X0[:, 3]], dim=1)
        rir_embedding = rir_embedding.repeat(1, 1, 1, X0.shape[-1])

        e0 = self.in_ch_lstm(X0)
        e0 = self.in_conv(torch.cat([e0, X0], 1))
        e1 = self.cfb_e1(torch.cat([e0, rir_embedding], dim=1))
        e2 = self.cfb_e2(torch.cat([e1, rir_embedding], dim=1))
        e3 = self.cfb_e3(torch.cat([e2, rir_embedding], dim=1))
        e4 = self.cfb_e4(torch.cat([e3, rir_embedding], dim=1))
        e5 = self.cfb_e5(torch.cat([e4, rir_embedding], dim=1))
