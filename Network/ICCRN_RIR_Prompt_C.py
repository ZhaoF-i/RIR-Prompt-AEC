...
class NET(nn.Module):
    def __init__(self, order=10, channels=20):
        ...
        self.in_ch_lstm = CH_LSTM_F(4 + 40, channels, channels)
        self.in_conv = nn.Conv2d(in_channels=4 + 40 + channels, out_channels=channels, kernel_size=(1, 1))
    ...
    # def forward(self, x):
        ...
        rir_embedding = comp_rir_channel_cat.repeat(1, 1, 1, X0.shape[-1])
        X0 = torch.cat([X0, rir_embedding], dim=1)
