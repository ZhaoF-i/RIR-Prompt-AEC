...
class MTFAANet(nn.Module):
    ...
    def __init__(self,
        ...
        self.mag_mask = nn.Conv2d(
            4, mag_f_dim, kernel_size=(3, 1), padding=(0, 0))
        self.real_mask = nn.Conv2d(4, 1, kernel_size=(3, 1), padding=(0, 0))
        self.imag_mask = nn.Conv2d(4, 1, kernel_size=(3, 1), padding=(0, 0))
    ...
    def forward(self, sigs):
        ...
        for idx in range(len(self.decoder_fu)):
            if out.shape[2] == encoder_out[-1-idx].shape[2]:
                out = self.decoder_fu[idx](out, encoder_out[-1-idx])
            else:
                out_emp = out[:, :, -encoder_out[-1-idx].shape[2]:, :]
                out = self.decoder_fu[idx](out_emp, encoder_out[-1-idx])
            out = self.decoder_bn[idx](out)
        # out = self.ERB.bank2amp(out)
        out = tf.pad(out, (0,0,1,0))

        # stage 1
        mag_mask = self.mag_mask(out)
        mag_pad = tf.pad(mag[:, None], [0, 0, (self.mag_f_dim-1)//2, (self.mag_f_dim-1)//2])
...
def test_nnet():
    ...
    nnet = MTFAANet(n_sig=2)
    inp = th.randn(1, 16000)
    mag, cspec, wav = nnet([inp, inp])
